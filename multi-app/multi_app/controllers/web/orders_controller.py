...
class OrdersController(BaseViewController[OrderService, OrdersView, OrderPlainSchema, OrderStrictSchema]):
    ...
    async def _build_view(self, translation: Translation, mode: ViewMode, event: ViewRequested) -> OrdersView:
        ...
        selected_order_id = self.__resolve_selected_order_id(event.record_id, orders)
    ...
    @staticmethod
    def __resolve_selected_order_id(record_id: int | None, orders: list[dict[str, Any]]) -> int | None:
        if not orders:
            return None
        if record_id is not None:
            for order in orders:
                if order.get("id") == record_id:
                    return record_id
