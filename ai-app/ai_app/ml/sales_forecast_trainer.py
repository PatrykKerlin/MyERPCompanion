...
class SalesForecastTrainer:
    ...
    def train(self, x_train: torch.Tensor, y_train: torch.Tensor) -> nn.Module:
        ...
        loss_fn = nn.MSELoss()
        train_x, train_y, val_x, val_y = SalesForecastTrainer._split_train_validation(x_train, y_train)
    ...
    @staticmethod
    def predict(model: nn.Module, x_predict: torch.Tensor) -> np.ndarray:
        with torch.no_grad():
