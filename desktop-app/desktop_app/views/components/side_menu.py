...
class SideMenu(ft.Container):
    ...
    def __add_module_controls(self, module: ModuleInputSchema) -> None:
        visible_endpoints = [endpoint for endpoint in module.endpoints if endpoint.in_menu]
        if not visible_endpoints:
            return

        label = self.__texts.get(module.key, module.key)
    ...
    def __add_endpoint_controls(self, endpoint) -> None:
        ...
        endpoint_label = self.__texts.get(endpoint.key, endpoint.key)
        if endpoint_label:
    ...
    def __add_list_tile(self, key: str, icon: str) -> None:
        label = self.__texts.get(key, key)
        self.__labels.append(label)
        self.__controls.append(
            ft.ListTile(
                ...
                dense=True,
                on_click=lambda _: self.__controller.on_menu_click(key),
            )
        )
